...
public class Tools {
  ...
  public JsonNode definitions() {
    ...
    ((com.fasterxml.jackson.databind.node.ObjectNode) input.path("properties"))
        .putObject("answerQuestion")
        .put("type", "string")
        .put("minLength", 1)
        .put("maxLength", 2000)
        .put(
            "description",
            "Only when the user also asks a question after edits: self-contained question with resolved names.");
    var tool =
  ...
  public Result execute(String name, JsonNode input) {
    return execute(name, input, store::apply);
  }

  public Result execute(
      String name, JsonNode input, java.util.function.Function<Plan, FamilyStore.Applied> apply) {
    try {
      require(
      ...
      var payload = (com.fasterxml.jackson.databind.node.ObjectNode) input.deepCopy();
      var question = payload.remove("answerQuestion");
      require(
          question == null
              || (question.isTextual()
                  && !question.asText().isBlank()
                  && question.asText().length() <= 2000),
          "INVALID_INPUT",
          "Invalid follow-up question.");
      var plan = json.treeToValue(payload, Plan.class);
      return new Result(false, apply.apply(plan));
    } catch (Invalid e) {
